...
const App: React.FC = () => {
  ...
  const handleReveal = () => {
    ...
    const updatedPlayers = players.map(p => ({
      ...p,
      // Automatically check if their selected index matches correct index
      lastAnswerCorrect: p.selectedIdx === correctIdx
    }));

    setPlayers(updatedPlayers);
    setGameState('REVEAL');
    // Optional: broadcast to players so they see 'Correct/Incorrect' immediately
  };
            ...
            <div className="glass p-12 rounded-[3rem] text-center border-2 border-yellow-400/30 shadow-2xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {players.map(p => (
                  <div 
                    key={p.id} 
                    className={`p-6 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${p.lastAnswerCorrect ? 'bg-green-500/20 border-green-500' : 'bg-red-500/10 border-red-500/30 opacity-70'}`}
                  >
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center ${p.lastAnswerCorrect ? 'bg-green-500' : 'bg-red-500/20'}`}>
