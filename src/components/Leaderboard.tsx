    ...
    <div className="max-w-2xl mx-auto p-6 space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 relative z-10">
      ...
      <div className="glass rounded-3xl overflow-hidden">
        {sortedPlayers.map((player, idx) => (
          <div key={player.id} className={`p-6 flex items-center justify-between border-b border-white/5 transition-all ${idx === 0 ? 'bg-yellow-400/10' : ''} ${idx === sortedPlayers.length - 1 ? 'border-b-0' : ''}`}>
            <div className="flex items-center gap-5">
              <div className="relative">
                {idx < 3 ? (<Medal className={`w-8 h-8 ${idx === 0 ? 'text-yellow-400' : idx === 1 ? 'text-slate-300' : 'text-amber-600'}`} />) : (<span className="text-white/40 font-bold w-8 text-center">{idx + 1}</span>)}
              </div>
              <div>
      ...
      <button onClick={onNext} className="w-full py-5 bg-white text-slate-900 rounded-2xl font-black text-xl hover:bg-yellow-400 hover:scale-[1.02] transition-all flex items-center justify-center gap-3">
