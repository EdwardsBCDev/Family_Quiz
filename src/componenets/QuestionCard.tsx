...
const QuestionCard: React.FC<QuestionCardProps> = ({ 
  question, questionNumber, totalQuestions, players, onReveal 
}) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in zoom-in duration-500 px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {players.map(player => (
            <div key={player.id} className="flex flex-col items-center space-y-4">
              <div className={`w-24 h-24 rounded-full border-4 flex items-center justify-center transition-all duration-500 ${player.hasAnswered ? 'bg-green-500/20 border-green-500 scale-110 shadow-[0_0_30px_rgba(34,197,94,0.3)]' : 'bg-white/5 border-white/20 animate-pulse'}`}>
                {player.hasAnswered ? (<CheckCircle2 size={48} className="text-green-500" />) : (<span className="text-4xl font-black text-white/20">{player.name[0]}</span>)}
              </div>
              <span className={`font-black text-xl tracking-tight transition-colors ${player.hasAnswered ? 'text-white' : 'text-white/30'}`}>{player.name.toUpperCase()}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex justify-center pt-8">
        <button onClick={onReveal} className={`px-24 py-8 rounded-[3rem] font-black text-4xl transition-all shadow-2xl ${allAnswered ? 'bg-white text-slate-950 hover:bg-yellow-400 hover:scale-105 active:scale-95' : 'bg-white/5 text-white/10 cursor-not-allowed grayscale'}`}>
