...
import { Send, CheckCircle2, Trophy, Wifi } from 'lucide-react';
  ...
  if (!connected) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center space-y-4">
        ...
        <h2 className="text-xl font-bold">Connecting to Edwards Family Quiz...</h2>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 p-4 flex flex-col space-y-4">
      ...
      {gameState === 'LOBBY' && (
        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6">
          ...
          <p className="text-white/60">Watch the big screen. The quiz will start shortly.</p>
        </div>
      )}
        ...
        <div className="flex-1 flex flex-col space-y-3 animate-in fade-in slide-in-from-bottom-4">
          ...
          <div className="grid grid-cols-1 gap-3 flex-1">
            {['A', 'B', 'C', 'D'].map((label, idx) => (
              <button
                ...
                className={`
                  flex-1 rounded-2xl text-4xl font-black transition-all border-4 flex items-center justify-center
                  ${selected === idx 
                    ? 'bg-yellow-400 border-yellow-200 text-slate-950 scale-95 shadow-inner' 
                    : selected === null 
                      ? 'bg-white/5 border-white/10 text-white active:scale-95 active:bg-white/10' 
          ...
          {selected !== null && (
            <div className="p-4 bg-green-500/20 border border-green-500/50 rounded-2xl text-center animate-pulse">
               <p className="text-sm font-bold text-green-400 uppercase tracking-widest">Locked In! Watch the Screen</p>
            </div>
          )}
        </div>
      )}
      ...
      {(gameState === 'REVEAL' || gameState === 'LEADERBOARD') && (
        <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
          ...
          <p className="text-white/60">Check the leaderboard on the main screen!</p>
        </div>
      )}
    </div>
  );
